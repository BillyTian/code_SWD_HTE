...
empirical.tIe <- NULL
for (i in 1:nrow(table)){
  empirical.tIe <- rbind(empirical.tIe, empirical_CS_ATE_unadj(nullcase=T, parameter=table[i,]))
}

empirical.power <- NULL
for (i in 1:nrow(table)){
  empirical.power <- rbind(empirical.power, empirical_CS_ATE_unadj(parameter=table[i,]))
}
